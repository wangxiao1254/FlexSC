...
public class ISAACEngine extends SecureRandomSpi {
	...
	public void engineNextBytes(byte[] bytes) {
		...
		if (rC > 0) {
			todo = (bytes.length - index) < (4 - rC) ? (bytes.length - index)
					: (4 - rC);
	...
	private static final int[] mask = new int[] { 0xFF000000, 0x00FF0000,
	...
	private static byte[] toByteArray(final int intValue) {
		int byteNum = (40 - numberOfLeadingZeros(intValue < 0 ? ~intValue
				: intValue)) / 8;
