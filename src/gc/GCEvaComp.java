...
public abstract class GCEvaComp extends GCCompEnv{
	...
	public GCSignal xor(GCSignal a, GCSignal b) {
		if (a.isPublic() && b.isPublic())
			return  ((a.v ^ b.v) ? new GCSignal(true, GCSignal.wid++):new GCSignal(false, GCSignal.wid++));
		else if (a.isPublic())
			return a.v ? not(b) : b;
		else if (b.isPublic())
			return b.v ? not(a) : a;
	...
	public GCSignal not(GCSignal a) {
		if (a.isPublic())
			return ((!a.v) ?new GCSignal(true, GCSignal.wid++):new GCSignal(false, GCSignal.wid++));
		else {
			return a;
		}
	}
